...
fun <T : Comparable<T>> merge(left: List<T>, right: List<T>): List<T> =
    sequence {
        ...
        while (lIndex < left.size || rIndex < right.size) {
            yield((right.takeIf { lIndex >= left.size || (rIndex < right.size && it[rIndex] < left[lIndex]) } ?: left)[rIndex.takeIf { lIndex < left.size } ?: lIndex++])
            if (lIndex == left.size) rIndex++ else if (rIndex == right.size || left[lIndex] <= right[rIndex]) lIndex++ else rIndex++
        }
    }.toList()
